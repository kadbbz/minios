...
def create_output_dir(root: Path) -> Path:
    out_dir = root / ".tmp" / f"docker-image-full-test-{timestamp_label()}"
...
def write_summary(root: Path, out_dir: Path, results: list[StepResult], started_at: datetime, ended_at: datetime) -> None:
    ...
    lines = [
        "MiniOS Docker Compose Full Test Report",
...
def prepare_fixture(root: Path, out_dir: Path) -> str:
    ...
    (input_dir / "model-config-template.md").write_text(
        "\n".join(
            [
                "# MiniOS Runtime Config",
                "",
                "- Primary Model: ",
                "- Backup Models: ",
                "- Selected Provider: ",
    ...
    (fixture_root / "file-payload.json").write_text(
        json.dumps(
            {
                "messageId": "msg-file-full-001",
                "sessionId": "sess-compose-full-001",
                "threadId": "thread-template",
                "text": "请严格保留 Markdown 结构，并填写所有配置字段后返回。",
                "attachments": [
                    {
                        "bucket": "agents-in",
                        "key": "agent-compose-full/sess-compose-full-001/thread-template/msg-file-full-001/model-config-template.md",
                        "name": "model-config-template.md",
                        "mediaType": "text/markdown",
                    }
                ],
                "traceId": "trace-file-full-001",
                "metadata": {
                    "workflow": "llm-template-fill"
                },
            },
            ensure_ascii=False,
            indent=2,
    ...
    return f"prepared full persistent fixture root at {fixture_root}"
...
def prepare_gateway_test_root(root: Path, out_dir: Path) -> str:
    ...
    shell = (
        ...
        "  fi\n"
        "done\n"
        f"aws --endpoint-url http://minio:9000 s3 cp {fixture_root_container}/input/model-config-template.md "
        "s3://agents-in/agent-compose-full/sess-compose-full-001/thread-template/msg-file-full-001/model-config-template.md "
        "--only-show-errors\n"
    )
    ...
    return result.stdout.strip() or "gateway full fixture and buckets prepared in persistent volume"
...
def runtime_llm_template_flow(root: Path, out_dir: Path) -> str:
    state = read_state(out_dir)
    container_name = state["gatewayContainer"]
    ...
    file_published = run_json_command(
        [
            ...
            "runtime", "publish",
            "--root", runtime_root_container,
            "--topic", "agents/file/inbound/agent-compose-full",
            f"{fixture_root_container}/file-payload.json",
        ],
        root,
        timeout=300,
    )
    require(file_published["ok"] is True, "full file publish failed in compose gateway")
    ...
    require(final_event["kind"] == "final", "full flow final event missing")
    require(len(final_event["attachments"]) == 1, "full flow should return one output attachment")
    output_attachment = final_event["attachments"][0]
    require(output_attachment["bucket"] == "agents-out", "full flow output should be uploaded to agents-out")

    turn_id = final_event["turnId"]
    local_outbox = f"{runtime_root_container}/data/agents/agent-compose-full/workspace/sessions/sess-compose-full-001/outbox/thread-template/{turn_id}/filled-model-config.md"
    outbox_check = run_command(["docker", "exec", container_name, "test", "-f", local_outbox], root, timeout=60)
    require(outbox_check.returncode == 0, "filled markdown missing from outbox")

    read_outbox = run_command(["docker", "exec", container_name, "cat", local_outbox], root, timeout=60)
    require(read_outbox.returncode == 0, "failed to read filled markdown")
    output_markdown = read_outbox.stdout
    (results_dir_host / "filled-model-config.md").write_text(output_markdown, encoding="utf-8")

    for needle in [
        "# MiniOS Runtime Config",
        "Primary Model: corp-openai/qwen3.5-plus",
        "Selected Provider: corp-openai",
        "Selected Model: qwen3.5-plus",
        "Provider API: openai-completions",
        "Provider Base URL: https://dashscope.aliyuncs.com/compatible-mode/v1",
        "API Key Env: OC_OPENAI_API_KEY",
        "generated-by: corp-openai/qwen3.5-plus",
    ]:
        require(needle in output_markdown, f"filled markdown missing expected content: {needle}")
    ...
    require(output_head.returncode == 0, "uploaded markdown object missing from MinIO")

    downloaded_from_minio = run_command(
        [
            "docker",
            "exec",
            container_name,
            "/bin/bash",
            "-lc",
            f"aws --endpoint-url http://minio:9000 s3 cp 's3://agents-out/{output_attachment['key']}' - --only-show-errors",
        ],
        root,
        timeout=120,
    )
    require(downloaded_from_minio.returncode == 0, "failed to read generated markdown from MinIO")
    require(downloaded_from_minio.stdout == output_markdown, "MinIO object content differs from outbox markdown")
    (results_dir_host / "filled-model-config-from-minio.md").write_text(downloaded_from_minio.stdout, encoding="utf-8")
    ...
    logs = run_json_command(
        [
            ...
            "--root", runtime_root_container,
            "--id", "agent-compose-full",
            "--session", "sess-compose-full-001",
            "--tail", "50",
        ],
        root,
        timeout=120,
    )
    require(logs["ok"] is True, "agents logs failed in full compose test")
    require(logs["result"]["count"] >= 3, "full compose logs should contain runtime events")
    ...
    require(usage_result.returncode == 0, f"usage command failed\nstdout={usage_result.stdout}\nstderr={usage_result.stderr}")
    usage_payload = json.loads(usage_result.stdout)
    require(isinstance(usage_payload, list), "usage command must return a json array")
    require(len(usage_payload) == 1, "usage result should contain one agent/session aggregate")
    usage_item = usage_payload[0]
    require(usage_item["agentId"] == "agent-compose-full", "usage agentId mismatch")
    require(usage_item["sessionId"] == "sess-compose-full-001", "usage sessionId mismatch")
    require(len(usage_item["usageByLlm"]) == 1, "usage should contain one llm aggregate")
    model_usage = usage_item["usageByLlm"][0]
    require(model_usage["name"] == "Qwen 3.5 Plus", "usage llm name mismatch")
    require(model_usage["requests"] >= 1, "usage requests should be >= 1")
    require(model_usage["totalTokens"] > 0, "usage totalTokens should be > 0")
    (results_dir_host / "usage.json").write_text(json.dumps(usage_payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")

    host_outbox = runtime_root_host / "data" / "agents" / "agent-compose-full" / "workspace" / "sessions" / "sess-compose-full-001" / "outbox" / "thread-template" / turn_id / "filled-model-config.md"
    require(host_outbox.is_file(), f"persistent generated markdown missing on host: {host_outbox}")

    state["lastTurnId"] = turn_id
    state["lastOutputObjectKey"] = output_attachment["key"]
    (out_dir / "state.json").write_text(json.dumps(state, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return "agent add/llm template fill/upload/logs passed with persistent inputs and outputs"
...
def restart_persistence(root: Path, out_dir: Path) -> str:
    ...
    info = run_json_command(
        ["docker", "exec", container_name, "node", "dist/cli.js", "agents", "info", "--root", runtime_root_container, "--id", "agent-compose-full"],
        root,
        timeout=120,
    )
    ...
    host_outbox = runtime_root_host / "data" / "agents" / "agent-compose-full" / "workspace" / "sessions" / "sess-compose-full-001" / "outbox" / "thread-template" / turn_id / "filled-model-config.md"
    require(host_outbox.is_file(), f"generated markdown lost after restart: {host_outbox}")
    ...
    return f"{wait_message}; persisted config and generated markdown survived docker restart"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Run full regression checks against a built MiniOS image through docker compose.")
    ...
    tests: list[tuple[str, Callable[[Path, Path], str]]] = [
        ...
        ("runtime_llm_template_flow", runtime_llm_template_flow),
